...
from dotenv import load_dotenv  # <-- 2. Import the 'load_dotenv' function

# --- Load environment variables from .env file ---
load_dotenv()  # <-- 3. Load the variables

# --- 1. Configuration ---
# --- The MONGO_URI is now loaded from your .env file
MONGO_URI = os.getenv("MONGO_URI")
DB_NAME = "blog_moderateur" # This variable is descriptive but the actual DB is set in the URI
...
try:
    if not MONGO_URI:
        raise Exception("La variable d'environnement MONGO_URI n'est pas définie. Assurez-vous d'avoir un fichier .env.")
    ...
    # get_default_database() will use the database specified in the MONGO_URI
    db = client.get_default_database() 
    collection = db[COLLECTION_NAME]
    print(f"Connexion à MongoDB réussie. Base de données : '{db.name}'.")
...
def scrape_article(article_url):
    ...
    try:
        thumbnail_url = soup.find('figure', class_='post-thumbnail').find('img')['src']
    except (AttributeError, KeyError):
        thumbnail_url = "URL de miniature non trouvée"

    try:
        # La catégorie est dans un 'span' avec la classe 'hat-link'
        subcategory = soup.find('span', class_='hat-link').get_text(strip=True)
    except AttributeError:
        subcategory = "Sous-catégorie non trouvée"

    try:
        summary = soup.find('p', class_='chapo').get_text(strip=True)
    except AttributeError:
